...
@endphp
<x-layouts.admin :pageTitle="'Edit Role'" :role="$roleLabel">
    <x-slot name="breadcrumb">
        ...
        <span class="text-slate-600">Edit {{ $roleModel->name }}</span>
    </x-slot>
    ...
    <div class="mx-auto w-full max-w-5xl space-y-6">
        <section class="admin-soft-ring rounded-3xl bg-gradient-to-r from-indigo-600 via-cyan-600 to-emerald-600 px-6 py-6 text-white sm:px-8">
            ...
            <h2 class="admin-display mt-2 text-2xl font-bold">Edit Role</h2>
            <p class="mt-2 max-w-2xl text-sm text-cyan-100">Update role details and adjust permission coverage.</p>
        </section>

        <div class="admin-panel rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.roles.update', $roleModel) }}">
                @csrf
                @method('PATCH')
