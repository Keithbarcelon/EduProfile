...
<head>
    @php
        ...
        $schoolName = trim((string) ($school->name ?? 'Tenant Portal'));
        $schoolInitial = strtoupper(substr($schoolName, 0, 1));
        $logoUrl = ($school && filled($school->logo_path))
            ? route('tenant.logo', ['v' => optional($school->updated_at)->timestamp])
            : null;
    @endphp
    <meta charset="UTF-8">
            ...
            <div class="text-center mb-4">
                @if($logoUrl)
                    <a href="{{ route('landing') }}" class="text-decoration-none">
                        <img src="{{ $logoUrl }}" alt="{{ $schoolName }} logo" class="ep-logo-img">
                    </a>
                @else
